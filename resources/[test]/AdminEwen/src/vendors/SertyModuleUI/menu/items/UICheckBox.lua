...
local function StyleCheckBox(Selected, Checked, Box, BoxSelect, OffSet)
    ...
    if Selected then
        if Checked then
            RenderSprite(SettingsCheckbox.Dictionary, SettingsCheckbox.Textures[Box], CurrentMenu.X + SettingsCheckbox.X + CurrentMenu.WidthOffset - OffSet, CurrentMenu.Y + SettingsCheckbox.Y + CurrentMenu.SubtitleHeight + SertyModuleUI.ItemOffset, SettingsCheckbox.Width, SettingsCheckbox.Height)
        else
            RenderSprite(SettingsCheckbox.Dictionary, SettingsCheckbox.Textures[1], CurrentMenu.X + SettingsCheckbox.X + CurrentMenu.WidthOffset - OffSet, CurrentMenu.Y + SettingsCheckbox.Y + CurrentMenu.SubtitleHeight + SertyModuleUI.ItemOffset, SettingsCheckbox.Width, SettingsCheckbox.Height)
        end
    else
        if Checked then
            RenderSprite(SettingsCheckbox.Dictionary, SettingsCheckbox.Textures[BoxSelect], CurrentMenu.X + SettingsCheckbox.X + CurrentMenu.WidthOffset - OffSet, CurrentMenu.Y + SettingsCheckbox.Y + CurrentMenu.SubtitleHeight + SertyModuleUI.ItemOffset, SettingsCheckbox.Width, SettingsCheckbox.Height)
        else
            RenderSprite(SettingsCheckbox.Dictionary, SettingsCheckbox.Textures[3], CurrentMenu.X + SettingsCheckbox.X + CurrentMenu.WidthOffset - OffSet, CurrentMenu.Y + SettingsCheckbox.Y + CurrentMenu.SubtitleHeight + SertyModuleUI.ItemOffset, SettingsCheckbox.Width, SettingsCheckbox.Height)
        end
    end
end
...
---@return nil
---@public
function SertyModuleUI.Checkbox(Label, Description, Checked, Style, Callback, onChecked, onUnchecked)
            ...
            if CurrentMenu.Pagination.Minimum <= Option and CurrentMenu.Pagination.Maximum >= Option then
                ...
                if type(Style) == "table" then
                    if Style.Enabled == true or Style.Enabled == nil then
                        ...
                        local LeftBadgeOffset = ((LeftBadge == SertyModuleUI.BadgeStyle.None or LeftBadge == nil) and 0 or 27)
                    ...
                    if Style.Style ~= nil then
                        if Style.Style == SertyModuleUI.CheckboxStyle.Tick then
                            StyleCheckBox(Selected, Checked, 2, 4, BoxOffset)
                        elseif Style.Style == SertyModuleUI.CheckboxStyle.Cross then
