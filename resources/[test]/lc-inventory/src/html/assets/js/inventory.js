...
window.addEventListener("message", function(event) {
    ...
    if (event.data.action == "open:Inv") {
        ...
        $.post("http://lc-inventory/cancel", JSON.stringify({}));
    ...
    $('#itemFast-1').droppable({
        drop: function(event, ui) {
            ...
            if (type === "normal" && (itemInventory === "main" || itemInventory === "fast")) {
                ...
                $.post("http://lc-inventory/PutIntoFast", JSON.stringify({
                    item: itemData,
                    slot: 1
                }));
            }
...
$(function() {
    ...
    $('#button\\.raccourci-1').click(function() {
        $.post('http://lc-inventory/category', JSON.stringify({
            type: 'all'
        }));
    });
...
$(document).ready(function() {
    ...
    $(document).keydown(function(event) {
        if (event.which === 9) { // 9 est le code de la touche TAB
            ...
            if (inventoryVisible) {
                ...
                $(".form-inv").fadeOut(100);
                inventoryVisible = false;
                $.post("http://lc-inventory/close",JSON.stringify())
    ...
    $('.middle-clothes-part-left .item-box').off().click(function(){
        $.post("http://lc-inventory/removeClothes",JSON.stringify({component:$(this).attr('id')}))
    })
    ...
    $('#useItem').droppable({
        ...
        drop: function(event, ui) {
            ...
            itemData = ui.draggable.data("item");
            if (itemData.usable) {
                $.post("http://lc-inventory/useItem", JSON.stringify({
                    item: itemData,
    ...
    $('#giveItem').droppable({
        ...
        drop: function(event, ui) {
            itemData = ui.draggable.data("item");
            $.post("http://lc-inventory/giveItem", JSON.stringify({
                item: itemData,
                // number: parseInt($("#count").val())
            }));
        }
    ...
    $('#renameItem').droppable({
        ...
        drop: function(event, ui) {
            itemData = ui.draggable.data("item");
            $.post("http://lc-inventory/renameItem", JSON.stringify({
                item: itemData,
                // number: parseInt($("#count").val())
            }));
        }
    ...
    $('#deleteItem').droppable({
        ...
        drop: function(event, ui) {
            itemData = ui.draggable.data("item");
            $.post("http://lc-inventory/deleteItem", JSON.stringify({
                item: itemData,
                // number: parseInt($("#count").val())
            }));
        }
    ...
    $('#left-inventory').droppable({
        drop: function(event, ui) {
            ...
            if (type === "trunk" && itemInventory === "second") {
                disableInventory(500);
                $.post("http://lc-inventory/TakeFromTrunk", JSON.stringify({
                    item: itemData,
                    // number: parseInt($("#count").val())
                }));
            } else if (type === "property" && itemInventory === "second") {
                disableInventory(500);
                $.post("http://lc-inventory/TakeFromProperty", JSON.stringify({
                    item: itemData,
                    // number: parseInt($("#count").val())
                }));
            } else if (type === "normal" && itemInventory === "fast") {
                disableInventory(500);
                $.post("http://lc-inventory/TakeFromFast", JSON.stringify({
                    item: itemData
                }));
            } else if (type === "vault" && itemInventory === "second") {
                disableInventory(500);
                $.post("http://lc-inventory/TakeFromVault", JSON.stringify({
                    item: itemData,
                    // number: parseInt($("#count").val())
                }));
            } else if (type === "player" && itemInventory === "second") {
                disableInventory(500);
                $.post("http://lc-inventory/TakeFromPlayer", JSON.stringify({
                    item: itemData,
                    // number: parseInt($("#count").val())
                }));
            }
    ...
    $('#right-inventory').droppable({
        drop: function(event, ui) {
            ...
            if (type === "trunk" && itemInventory === "main") {
                disableInventory(500);
                $.post("http://lc-inventory/PutIntoTrunk", JSON.stringify({
                    item: itemData,
                }));
            } else if (type === "property" && itemInventory === "main") {
                disableInventory(500);
                $.post("http://lc-inventory/PutIntoProperty", JSON.stringify({
                    item: itemData,
                }));
                ...
                disableInventory(500);
                $.post("http://lc-inventory/PutIntoVault", JSON.stringify({
                    item: itemData,
                    // number: parseInt($("#count").val())
                }));
            } else if (type === "player" && itemInventory === "main") {
                disableInventory(500);
                $.post("http://lc-inventory/PutIntoPlayer", JSON.stringify({
                    item: itemData,
                }));
            }
        }
    ...
    document.getElementById("textArea").addEventListener("keyup", function(event) {
        ...
        $.post("http://lc-inventory/send", JSON.stringify({"text": document.getElementById("textArea").value.trim()}));
        document.getElementById("textArea").value = "";
        $("#keyboard-input").hide();
        }
    ...
    $('#button\\.showButton').click(function(){
        $.post("http://lc-inventory/giveCard", JSON.stringify({
            info: idcard,
            type: idcardType
        }));
        $(".menu").hide();
    ...
    $('#button\\.chooseButton').click(function(){
        ...
        $.post("http://lc-inventory/lookCard", JSON.stringify({
            info: idcard,
            type: idcardType
        }));
        $(".menu").hide();
...
$(document).mousedown(function(event) {
    ...
    itemInventory = $(event.target).data("lc-inventory");
