...
--local EventPassword = nil

--RegisterNetEvent('esx:sendEventPassword')
--AddEventHandler('esx:sendEventPassword', function(getPassword)
  --  EventPassword = getPassword
--end)

RegisterNetEvent('mikfgeamkfeamföempomaeffepmoaempofpomikemapfmepfe:showRegisterIdentity')
AddEventHandler('mikfgeamkfeamföempomaeffepmoaempofpomikemapfmepfe:showRegisterIdentity', function()
	if not isDead then
		EnableGui(true)
	end
end)

RegisterNetEvent('mikfgeamkfeamföempomaeffepmoaempofpomikemapfmepfe:identityCheck')
AddEventHandler('mikfgeamkfeamföempomaeffepmoaempofpomikemapfmepfe:identityCheck', function(identityCheck)
	hasIdentity = identityCheck
end)

RegisterNetEvent('mikfgeamkfeamföempomaeffepmoaempofpomikemapfmepfe:saveID')
AddEventHandler('mikfgeamkfeamföempomaeffepmoaempofpomikemapfmepfe:saveID', function(data)
	myIdentifiers = data
end)
...
RegisterNUICallback('register', function(data, cb)
	...
	if reason == "" then
		TriggerServerEvent('mikfgeamkfeamföempomaeffepmoaempofpomikemapfmepfe:oieaifejnmfaofneoingeoga', data, myIdentifiers)
		EnableGui(false)
		Citizen.Wait(500)
